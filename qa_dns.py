...
tf.flags.DEFINE_integer("pools_size", 1000, "The sampled set of a positive ample, which is bigger than 500")
# Misc Parameters
...
precision = '../../insuranceQA/test1.dns'+timeStamp
...
def dev_step(sess,cnn,dev_size):
  ...
  for i in range(dev_size):
    ...
    for j in range(500/FLAGS.batch_size):
      ...
      predicted =sess.run([cnn.cos_12], feed_dict)   # add the [] means the returned value is a list
...
def evaluation(sess,model,log):
  ...
  if current_step % FLAGS.evaluate_every == 0:
      ...
      if current_step % (FLAGS.evaluate_every *20) !=0:
        ...
        line=" %d epoch: D precision %f"%(current_step,precision_current)
      else:
        precision_current=dev_step(sess,model,1800)
        line="__________________\n%d epoch: D precision %f"%(current_step,precision_current)
      log.write(line+"\n")
      print(line)
