...
class Generator():
    def __init__(self, sequence_length, batch_size,vocab_size, embedding_size,filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        # self.pred_score= self.cos_13
        margin = tf.constant(1, shape=[batch_size], dtype=tf.float32)

        self.pred_score= tf.sub(margin, tf.sub(self.cos_12, self.cos_13)) ## attention
        self.gan_loss = -tf.reduce_mean(tf.log(self.pred_score) * self.reward)
