...
def load_data_pair(vocab,pools,raw,size):
    x_train_1 = []
    x_train_2 = []
    x_train_3 = []
    for i in range(0, size):
        ...
        x_train_1.append(encode_sent(vocab, items[2], 100))
        x_train_2.append(encode_sent(vocab, items[3], 100))
        x_train_3.append(encode_sent(vocab, pools[i], 100))
    return np.array(x_train_1), np.array(x_train_2), np.array(x_train_3)
def load_data_pair1(vocab,pools,items):
    x_train_1 = []
    x_train_2 = []
    x_train_3 = []
...
def main():
    ...
    # print vocab
    # print len(raw)
    testList, vectors = load_test_and_vectors()
    
    # x,y,z=load_data_6(vocab, alist, raw, 100)
    print len(testList)
    count=0
    for i in range(101):
        if int(testList[500*i+1].split()[0])==1:
            count+=1
    # print testList[500*4:2004]
    print count
