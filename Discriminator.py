...
class Discriminator():
    def __init__(self, sequence_length, batch_size,vocab_size, embedding_size,filter_sizes, num_filters, l2_reg_lambda=0.0,learning_rate=1e-2):
        # InsQACNN.__init__(self, sequence_length, batch_size,vocab_size, embedding_size,filter_sizes, num_filters, l2_reg_lambda=0.0)
        ...
        l2_reg_lambda=0#0.1 /batch_size
        ...
        with tf.name_scope("loss"):
            ...
            # maxreward = tf.constant(0.2, shape=[batch_size], dtype=tf.float32)
            # self.reward_losses =  tf.minimum(tf.maximum(zero, tf.sub(margin, tf.sub(self.cos_12, self.cos_13))),maxreward)
            self.reward_losses =  tf.maximum(zero, tf.sub(margin, tf.sub(self.cos_12, self.cos_13)))
            self.reward_loss = tf.reduce_sum(self.losses) + l2_reg_lambda * l2_loss
            self.neg_reward = (tf.sigmoid(self.reward_losses)) # dele tf.log
        ...
        margin = tf.constant(1, shape=[batch_size], dtype=tf.float32)

        self.pred_score= tf.sub(margin, tf.sub(self.cos_12, self.cos_13))

        # self.gan_loss = -tf.reduce_mean(tf.log(self.pred_score) * self.neg_reward)
        # optimizer = tf.train.GradientDescentOptimizer(self.learning_rate)

        # self.gan_updates = optimizer.minimize(self.loss)
